...
public class DashboardGenerator {
    ...
    private void prepareDashboards(LocalDateTime startingTimestamp, List<Transaction> transactions) {
        ...
        for (Transaction tx : transactions) {
            // Считаем количество транзакций по статусам для четвертого дашборда
            if (tx.getStatus().getStatusID() == 4) {
                cancelledTransactions++;
            } else if (tx.getStatus().getStatusID() == 5) {
                completedTransactions++;
            }

            // Пропускаем транзакции, которые не завершены (status_id != 5)
            ...
            if (tx.getStatus().getStatusID() == 4) cancelledTransactions++;
            else if (tx.getStatus().getStatusID() == 5) completedTransactions++;
